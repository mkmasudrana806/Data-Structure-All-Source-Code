...
//****************************************** below we inset a node at beggining ********************************************


    newnode = (struct node*) malloc(sizeof(struct node));//here we create a newnode
    printf("enter value to insert\n");
    scanf("%d",&newnode->data);// we store value to the newnode data part
    newnode->next=head; //we assign newnode next part to head.because previous head point korshe linked list k.
    head=newnode; //ekhane head = newnode diesi.akhn head ei new node k point korbe,r ei new node ager linked list k point k korbe.
    printf("after inseting a new node\n");
    while(newnode!=0)
    {
